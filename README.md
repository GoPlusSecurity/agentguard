...
> **DSH Runtime Phase 2A boundary:** the integration now observes DSH's native `tools/pre-execute` lifecycle and evaluates recognized actions through the same local/Cloud policy resolver and OSS `ActionScanner` used by AgentGuard's other runtime hosts. It is deliberately audit-only: evaluated `warn`, `require_approval`, and `block` decisions are recorded in `~/.agentguard/audit.jsonl` but do not change DSH execution. Source-plugin attribution remains `unknown` until DSH exposes a reliable ownership field. See the [DSH runtime guide](docs/dsh-runtime.md).
