...
export function renderDshMarkdown(report: DshPluginScanReport): string {
  ...
  const findings = report.findings.length > 0
    ? report.findings.map(finding => {
      ...
      return `| ${finding.severity.toUpperCase()} | ${markdownEscape(finding.ruleId)} | ${markdownEscape(location)} | ${markdownEscape(finding.message)} |`;
    }).join('\n')
    : '| — | — | — | No findings |';
...
**Recommendation:** ${RECOMMENDATIONS[report.installRecommendation]}
...
| Severity | Rule | Location | Explanation |
|---|---|---|---|
${findings}
  ...
  const findings = report.findings.length > 0
    ? report.findings.map(finding => `
      <article class="finding">
        ...
        <div><strong>${htmlEscape(finding.ruleId)}</strong><p>${htmlEscape(finding.message)}</p><code>${htmlEscape(finding.file)}${finding.line ? `:${finding.line}` : ''}</code>${finding.snippet ? `<pre>${htmlEscape(finding.snippet)}</pre>` : ''}</div>
      </article>`).join('')
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; background:radial-gradient(circle at 80% 0,#172c4b 0,transparent 36%),var(--bg); color:var(--text); font:15px/1.55 Inter,ui-sans-serif,system-ui,sans-serif; }
    ...
    .risk { display:inline-flex; margin-top:18px; padding:7px 12px; border-radius:999px; font-weight:800; text-transform:uppercase; background:color-mix(in srgb,var(--${risk}) 18%,transparent); color:var(--${risk}); border:1px solid color-mix(in srgb,var(--${risk}) 42%,transparent); }
...
<body><main>
  <header class="hero"><div class="eyebrow">AgentGuard for DSH</div><h1>${htmlEscape(report.identity.name)}</h1><p class="summary">${htmlEscape(report.summary)}</p><span class="risk">${risk} risk</span></header>
  <div class="grid">
  ...
  <div class="grid">
    ...
    <section class="recommendation"><h2>Install recommendation</h2><strong>${htmlEscape(RECOMMENDATIONS[report.installRecommendation])}</strong>${report.harmlessMismatch ? '<p>Looks harmless, but requests elevated capabilities.</p>' : ''}</section>
